 		...
 		if ($nbMatches > 0) {
 			...
 			if (isset($tabParams['data-store'], $tabParams['data-offset']) === true) {
 				...
 				if (isset($tabParams['height'], $tabParams['width']) === true) {
 					return \CHtml::image(\Yii::app()->getRequest()->getBaseUrl().'/'.Image::create($this->prop($tabParams['data-store'], $tabParams['data-offset']))->setRatio(false)->resize($tabParams['width'], $tabParams['height'])->getUrl());
 				} else {
 					return \CHtml::image(\Yii::app()->getRequest()->getBaseUrl().'/'.Image::create($this->prop($tabParams['data-store'], $tabParams['data-offset']))->getUrl());
 				}
 			} else {
 				return $matches[0];
 			}
	 				...
	 				if(is_array($modelCfg) === true) {
	 					...
	 					if((isset($modelCfg['rules']) === true) && (is_array($modelCfg['rules']) === true)) {
	 						foreach($modelCfg['rules'] as $rule) {
	 							...
	 							if($this->_exceptScenarios !== null) {
	 								$except = $this->_exceptScenarios;
	 								if(isset($rule['except']) === true) {
	 									$except = is_array($rule['except'])?implode(',', $rule['except']):$rule['except'];
	 									$except = $except.','.$this->_exceptScenarios;
	 								}
	 								$rule['except'] = $except;
	 							}
