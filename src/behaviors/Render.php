 ...
 *                    'class' => 'sweelix\yii1\behaviors\Render',
 *                ),
 *            );
 *        }
 *    }
 ...
 * @since     1.1
 */
class Render extends CBehavior
{
    ...
    /**
     * Render pure json to ease ws communication
     *
    ...
    {
        $metadata = null;
        if (isset($headers['metadata']) === true) {
            $metadata = $headers['metadata'];
            $newData = array('content' => $data, 'metadata' => $metadata);
            unset($headers['metadata']);
            $data = $newData;
        }
        ...
        if ((Yii::app()->getRequest()->getRequestType() === 'GET')
            || (Yii::app()->getRequest()->getRequestType() === 'HEAD')) {
            $this->sendEtagHeader($data);
        }
        self::sendHeader($httpCode);

        if ((Yii::app()->getRequest()->getRequestType() !== 'HEAD') && ($httpCode !== 204) && ($data !== null)) {
    ...
    protected function sendEtagHeader($data)
    {
        if (isset(Yii::app()->params['etagKey']) == true) {
            $etagKey = Yii::app()->params['etagKey'];
        } else {
            $etagKey = 'generated by sweekit';
        }
    ...
    protected function getPrefix()
    {
        if ($this->prefix === null) {
            ...
            $this->prefix = preg_replace('/[^0-9a-z-_]/i', '', $prefix);
        }
        return $this->prefix;
    }
