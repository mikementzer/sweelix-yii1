...
{
    public static $targetPath = 'application.runtime.sweelix.upload';
                ...
                if (is_array($value) == true) {
                    foreach ($value as $idx => $data) {
                        ...
                        if ((file_exists($myFile) === true) && (is_file($myFile) == true)) {
                            ...
                            self::$_files[$name][$name . '_' . $idx] = new static(
                                $data,
                                $myFile,
                                $fileInfo['extension'],
                                filesize($myFile)
                ...
                if (is_array($value) == true) {
                    ...
                    foreach ($value as $idx => $data) {
                        ...
                        if (($myFile !== false) && (file_exists($myFile) === true) && (is_file($myFile) == true)) {
                            ...
                            self::$_files[$infos['class']][$infos['attribute']][$testName . '_' . $idx] = new static($data,
                                $myFile, $fileInfo['extension'], filesize($myFile), $infos['class'],
                                $infos['attribute']);
                        }
                    }
                } else {
                    ...
                    if (($myFile !== false) && (file_exists($myFile) === true) && (is_file($myFile) == true)) {
                        ...
                        self::$_files[$infos['class']][$infos['attribute']][$testName] = new static($value, $myFile,
                            $fileInfo['extension'], filesize($myFile), $infos['class'], $infos['attribute']);
                    }
                }
            } elseif (is_array($value) == true) {
     ...
     * @since  1.1.0
     */
    public function __construct($name, $tempName, $extension, $size, $model=null, $attribute=null)
    {
        $this->_name = $name;
