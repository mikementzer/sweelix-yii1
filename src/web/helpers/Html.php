 ...
 * @category  helpers
 * @package   sweelix.yii1.web.helpers
 */

namespace sweelix\yii1\web\helpers;

use sweelix\yii1\web\UploadedFile;

/**
 * This Html class override CHtml class to
 * allow the use of javascript elements
 *
 * samples :
 * Methods to generate code to use as url (href="")
 * <code>
 *    $urlRaiseEvent = Html::raiseEventUrl($eventName, $parameters, $context);
 *    $urlRaiseEvent = Html::raiseRedirectUrl($url);
 *    $urlRaiseEvent = Html::raiseOpenShadowboxUrl($url, $shadowBoxOptions);
 *    $urlRaiseEvent = Html::raiseCloseShadowboxUrl();
 * </code>
 *
 * Methods to generate code to use in script code
 * <code>
 *    $jsRaiseEvent = Html::raiseEvent($eventName, $parameters, $context);
 *    $jsRaiseEvent = Html::raiseRedirect($url);
 *    $jsRaiseEvent = Html::raiseOpenShadowbox($url, $shadowBoxOptions);
 *    $jsRaiseEvent = Html::raiseCloseShadowbox();
 * </code>
 *
 * Method to register an event using the clientScript
 * <code>
 *    Html::registerEvent($eventName, $action, $context);
 * </code>
 *
 * Method to generate js code to register an event manually
 * <code>
 *    $jsRaiseEvent = Html::registerEventScript($eventName, $action, $context);
 * </code>
 *
 * Method to generate async file upload
 * <code>
 *    Html::asyncFileUpload($model, $attribute, $options);
 * </code>
 *
 * @author    Philippe Gaultier <pgaultier@sweelix.net>
 * @copyright 2010-2014 Sweelix
...
class Html extends \CHtml
{
     ...
     */
    public static function listDataFromActiveDataProvider(
        $activeDataProvider,
        $valueField,
        $textField,
        $groupField = ''
    ...
    public static function activeAsyncFileUpload($model, $attribute, $htmlOptions = array())
    {
        ...
        $value = UploadedFile::getInstances($model, $attribute);
        ...
        if ($fileList !== null) {
            ...
            if (isset($model->resourcePath) === true) {
                ...
                $resourcePath = \Yii::getPathOfAlias('webroot');
            }
            ...
            foreach ($fileList as $element) {
                if ((strncasecmp('tmp://', $element, 6) !== 0) && (empty($element) === false)) {
                    $realFile = $resourcePath . DIRECTORY_SEPARATOR . pathinfo($element, PATHINFO_BASENAME);
                    if (file_exists($realFile) === true) {
                        $fileInfo = pathinfo($realFile);
                        $value[] = new UploadedFile($fileInfo['basename'], $realFile, $fileInfo['extension'],
                            filesize($realFile), $model, $attribute);
                    }
                }
            }
        }
    ...
    protected static function renderAsyncFileUpload($values, $htmlOptions, $config)
    {
        if (is_array($values) == true) {
            ...
            $config['uploadedFiles'][] = array(
                'fileName' => $values->getName(),
                'fileSize' => $values->getSize(),
                'status' => true
            );
        }
    ...
    protected static function prepareAsyncFileUpload(&$htmlOptions)
    {
        ...
        if (isset($htmlOptions['config']) == true) {
            if (isset($htmlOptions['config']['urlPreview']) && is_array($htmlOptions['config']['urlPreview']) === true) {
                $htmlOptions['config']['urlPreview'] = array_merge($htmlOptions['config']['urlPreview'],
                    array('id' => $htmlOptions['id'], 'key' => \Yii::app()->getSession()->getSessionId()));
            }
            if (isset($htmlOptions['config']['urlDelete']) && is_array($htmlOptions['config']['urlDelete']) === true) {
                $htmlOptions['config']['urlDelete'] = array_merge($htmlOptions['config']['urlDelete'],
                    array('id' => $htmlOptions['id'], 'key' => \Yii::app()->getSession()->getSessionId()));
            }
            if (isset($htmlOptions['config']['url']) && is_array($htmlOptions['config']['url']) === true) {
                $htmlOptions['config']['url'] = array_merge($htmlOptions['config']['url'],
                    array('id' => $htmlOptions['id'], 'key' => \Yii::app()->getSession()->getSessionId()));
            }
    ...
    public static function ajaxSubmitHandler($target)
    {
        ...
        \Yii::app()->getClientScript()->registerScript($scriptName, self::ajaxSubmitHandlerScript($target),
    ...
    {
        \Yii::app()->getClientScript()->registerSweelixScript('ajax');
        return self::raiseEvent('ajaxRefreshHandler', array(
